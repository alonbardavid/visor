  ...
  .provider("visor",[function(){
      ...
      config.doAfterManualAuthentication = ["$location",function($location){
          //$location.url($location.search().next || config.homeRoute);
           ...
           var next_is_dashboard = next && next.length >= "dashboard".length && next.substr(next.length - "dashboard".length) == "dashboard"
           if ( next && !next_is_dashboard ){
               $location.url(next || config.homeRoute);
           }
           else {
               $window.location.assign(config.homeRoute);
           }
      }];
      /**
       * @ngdoc function
